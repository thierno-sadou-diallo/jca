    ...
    <section class="admin-detail-grid">
        <article class="admin-panel">
            <div class="admin-panel-head">
                <h2>Creer un dossier immigration</h2>
            ...
            <form class="lead-form admin-form" method="post" action="{{ route('admin.immigration-cases.store') }}">
                ...
                @endif
                <label>Programme<input name="program_type" value="{{ old('program_type', $lead?->topic ?: 'Residence permanente') }}" required></label>
                <div class="form-grid">
                ...
                <label>Note initiale<textarea name="note" rows="5" placeholder="Resume de la situation, pieces attendues, prochaine etape...">{{ old('note') }}</textarea></label>
                @if ($errors->any())
                    <p class="form-note" data-state="error">{{ $errors->first() }}</p>
                @endif
                <button class="button primary" type="submit">Creer le dossier</button>
            </form>
        </article>
